...
export function createSlideCreatorAgent(provider: string = 'claude', modelName: string = 'claude-opus-4-20250514') {
...
2.  **計画立案・分析**: \`claudeAnalysisTool\` を使用し、収集したデータとユーザーから提供された情報（農地面積など）を基に、年間の作付計画や収支シミュレーションを構造化された形式（表形式のマークダウンなど）で生成します。
3.  **保存**: 生成した計画を、\`claudeFileTool\` を使って \`einou-keikaku.md\` のようなファイル名で保存し、ユーザーに報告します。
...
1.  **コンセプト定義**: \`claudeAnalysisTool\` を使い、ユーザーとの対話を通じて、商品の特徴、ターゲット顧客、提供価値（USP）などを明確化し、プレゼンテーションの骨子となるストーリーを作成します。
2.  **コンテンツ生成**:
...
1.  **情報抽出**: \`claudeAnalysisTool\` を使用し、日報のテキストから「作業内容」「使用した機材」「気づき」「課題」などの重要な情報を構造化データとして抽出します。
2.  **ナレッジ蓄積**: \`claudeFileTool\` の \`append\` 操作を使い、抽出した情報を \`knowledge-base.md\` というファイルに追記していきます。これにより、継続的に業務マニュアルやナレッジベースを構築します。
...
- \`claude-analysis\`: 包括的なAI搭載のコード支援ツール。重要：このツールを使用する際は、必ず'operation'フィールドを指定してください。利用可能な操作：
  - **analyze**: コードを分析し、問題、メトリクス、提案を提示します。例：{"operation": "analyze", "code": "your code", "language": "javascript"}
  - **generate**: 仕様に基づいて新しいコードを生成します。例：{"operation": "generate", "specification": "create a REST API", "language": "python"}
  - **review**: コードの品質をレビューし、フィードバックを提供します。例：{"operation": "review", "code": "your code", "reviewType": "comprehensive"}
  - **refactor**: 既存のコード構造を改善します。例：{"operation": "refactor", "code": "your code", "refactorType": "optimize"}
  - **generate-tests**: コードの単体テストを作成します。例：{"operation": "generate-tests", "code": "your code", "testFramework": "jest"}
  - **generate-docs**: コードのドキュメントを生成します。例：{"operation": "generate-docs", "code": "your code", "format": "markdown"}
- \`claude-file\`: プロジェクト内のファイルを読み込み、書き込み、追記、削除します。操作："read", "write", "append", "delete"。例：{"operation": "read", "filePath": "src/index.ts"}
- \`claude-auto-edit\`: Claude分析とファイル編集を組み合わせます。ファイルを自動的に分析・修正します。操作：
  - **analyze-and-fix**: コードを分析し、修正を適用します。例：{"operation": "analyze-and-fix", "filePath": "src/component.ts"}
  - **refactor-and-apply**: コードをリファクタリングし、変更を保存します。例：{"operation": "refactor-and-apply", "filePath": "src/utils.js", "refactorType": "optimize"}
  - **generate-and-save**: 新しいコードを生成し、ファイルに保存します。例：{"operation": "generate-and-save", "filePath": "src/newFeature.ts", "specification": "create a user authentication module"}
- \`claude-project-analyzer\`: Claudeの洞察を用いてプロジェクトのディレクトリ構造を分析します。操作：
  - **structure**: ディレクトリ構造のみをスキャンします。例：{"operation": "structure", "maxDepth": 3}
  - **summary**: プロジェクトの統計と概要を生成します。例：{"operation": "summary", "includeHidden": false}
  - **analyze**: アーキテクチャ、パターン、推奨事項に関するClaudeの洞察を含む完全な分析。例：{"operation": "analyze", "analysisType": "comprehensive"}
